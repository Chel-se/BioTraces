/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { PassportDetailWhereUniqueInput } from "./PassportDetailWhereUniqueInput";
import { ValidateNested } from "class-validator";
import { Type } from "class-transformer";
import { PassportDetailUpdateInput } from "./PassportDetailUpdateInput";

@ArgsType()
class UpdatePassportDetailArgs {
  @ApiProperty({
    required: true,
    type: () => PassportDetailWhereUniqueInput,
  })
  @ValidateNested()
  @Type(() => PassportDetailWhereUniqueInput)
  @Field(() => PassportDetailWhereUniqueInput, { nullable: false })
  where!: PassportDetailWhereUniqueInput;

  @ApiProperty({
    required: true,
    type: () => PassportDetailUpdateInput,
  })
  @ValidateNested()
  @Type(() => PassportDetailUpdateInput)
  @Field(() => PassportDetailUpdateInput, { nullable: false })
  data!: PassportDetailUpdateInput;
}

export { UpdatePassportDetailArgs as UpdatePassportDetailArgs };
